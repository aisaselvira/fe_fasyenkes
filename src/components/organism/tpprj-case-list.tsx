...
import {mapToTPPRJCase} from "@/pages/services/simulation/index";
import {Loader2} from "lucide-react";
import type {TPPRJCase} from "@/pages/services/simulation/types";
...
export function TPPRJCaseList({searchQuery = ""}: TPPRJCaseListProps) {
    const router = useRouter();
    const [currentPage, setCurrentPage] = useState(1);
    const [filteredCases, setFilteredCases] = useState<TPPRJCase[]>([]);
    const [cases, setCases] = useState<TPPRJCase[]>([]);
    ...
    const handleStartClick = (caseId: number) => {
        if (isAuthenticated()) {
            router.push(`/user/simulation/simulation-detail-case/tpprj/${caseId}`);
        } else {
            redirectToLogin();
        }
    };
    ...
    const fetchCases = async () => {
        ...
        try {
            ...
            if (response.error) {
                ...
                const mappedCases = response.data.data.map((record) => mapToTPPRJCase(record));
